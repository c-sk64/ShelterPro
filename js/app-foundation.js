(function(){
 ...
 function applyJobStatus(id,status,advance=false){if(!id)return;saveStatus(String(id),status);renderJobs();renderRouteManager();updateHome();if(advance){const next=nextJobId();if(next){selectSite(next,true);if(typeof currentDriveSite!=='undefined')currentDriveSite=site(next);if(typeof updateDrivingView==='function')updateDrivingView();}else alert('All jobs in today’s route are complete or skipped.');}}
 function startWorkflow(){const id=nextJobId();if(!id){alert('Import Today’s Jobs or add shelters to the route first.');return}closeHome();selectSite(id,true);if(typeof currentDriveSite!=='undefined')currentDriveSite=site(id);setTimeout(()=>startDriving(),120);}
 ...
 function addRoute(id){const s=site(id);if(!s){alert('Site '+id+' was not found.');return}if(!route.includes(s.id))route.push(s.id);save(K.route,route);renderRouteManager();}
 function renderJobs(){
